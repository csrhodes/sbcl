...
(locally (declare (optimize safety))
  (defclass typed-slot-safe-ctors ()
    ((slot :initarg :slot :type integer :initform 2.5)))
  (defun make-tssc (slot)
    (make-instance 'typed-slot-safe-ctors :slot slot))
  (defun make-tssc-initform ()
    (make-instance 'typed-slot-safe-ctors))
  (defun make-tssc-constant-integer ()
    (make-instance 'typed-slot-safe-ctors :slot 3))
  (defun make-tssc-constant-float ()
    (make-instance 'typed-slot-safe-ctors :slot 3.5)))

(with-test (:name (:typed-slot-safe-ctors :no-error))
  (assert (eql (slot-value (make-tssc 0) 'slot) 0)))
(with-test (:name (:typed-slot-safe-ctors type-error))
  (assert-error (make-tssc 1.5) type-error))
(with-test (:name (:typed-slot-safe-ctors type-error use-value))
  (handler-bind ((type-error (lambda (c)
                               (assert (eql (type-error-datum c) -1.5))
                               (use-value 2 c))))
    (assert (eql (slot-value (make-tssc -1.5) 'slot) 2))))
(with-test (:name (:typed-slot-safe-ctors :initform type-error))
  (assert-error (make-tssc-initform) type-error))
(with-test (:name (:typed-slot-safe-ctors :initform type-error use-value)
            :fails-on :sbcl)
  (handler-bind ((type-error (lambda (c)
                               (assert (eql (type-error-datum c) 2.5))
                               (use-value 2 c))))
    (assert (eql (slot-value (make-tssc-initform) 'slot) 2))))
(with-test (:name (:typed-slot-safe-ctors :constant-integer :no-error))
  (handler-bind ((type-error (lambda (c) (use-value 42 c))))
    (assert (eql (slot-value (make-tssc-constant-integer) 'slot) 3))))
(with-test (:name (:typed-slot-safe-ctors :constant-float type-error))
  (assert-error (make-tssc-constant-float) type-error))
(with-test (:name (:typed-slot-safe-ctors :constant-float type-error use-value))
  (handler-bind ((type-error (lambda (c)
                               (assert (eql (type-error-datum c) 3.5))
                               (use-value 4 c))))
    (assert (eql (slot-value (make-tssc-constant-float) 'slot) 4))))

(locally (declare (optimize safety))
  (defclass typed-slot-safe-ctors ()
    ((slot :initarg :slot :type integer))
    (:default-initargs :slot 4.5))
  (defun make-tssc-default-initarg ()
    (make-instance 'typed-slot-safe-ctors)))

(let ((ctor (gethash '(sb-pcl::ctor typed-slot-safe-ctors t) sb-pcl::*all-ctors*)))
  (assert (typep ctor 'sb-pcl::ctor))
  (assert (eql (sb-pcl::ctor-state ctor) 'sb-pcl::initial)))

(with-test (:name (:typed-slot-safe-ctors :default-initargs type-error))
  (assert-error (make-tssc-default-initarg)))
(with-test (:name (:typed-slot-safe-ctors :default-initargs type-error use-value)
            :fails-on :sbcl)
  (handler-bind ((type-error (lambda (c)
                               (assert (eql (type-error-datum c) 4.5))
                               (use-value 5 c))))
    (assert (eql (slot-value (make-tssc-default-initarg) 'slot) 5))))
