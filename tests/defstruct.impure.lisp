...
(let ((a 0))
  (defstruct (boa-constructor-which-env
               (:constructor make-bcwe (a))
               (:conc-name bcwe-))
    (a nil)
    (b (incf a))))
(with-test (:name (:boa-constructor :out-of-line :env))
  (let ((struct1 (make-bcwe 11))
        (struct2 (make-bcwe 22)))
    (assert (eql (bcwe-a struct1) 11))
    (assert (eql (bcwe-b struct1) 1))
    (assert (eql (bcwe-a struct2) 22))
    (assert (eql (bcwe-b struct2) 2))))

(let ((a 0))
  (defstruct (boa-constructor-which-env-optional
               (:constructor make-bcweo (a &optional b))
               (:conc-name bcweo-))
    (a nil)
    (b (incf a))))
(with-test (:name (:boa-constructor :out-of-line :env &optional)
                  :fails-on :sbcl)
  (let ((struct1 (make-bcweo 11))
        (struct2 (make-bcweo 22)))
    (assert (eql (bcweo-a struct1) 11))
    (assert (eql (bcweo-b struct1) 1))
    (assert (eql (bcweo-a struct2) 22))
    (assert (eql (bcweo-b struct2) 2))))
