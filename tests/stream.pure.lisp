...
;;; MAKE-OCTETS-INPUT-STREAM and WITH-INPUT-FROM-OCTETS

(with-test (:name (:make-octets-input-stream file-position))
  (let* ((octets (map '(vector (unsigned-byte 8)) 'digit-char-p "123"))
         (stream (make-octets-input-stream octets)))
    (assert (= (read-byte stream) 1))
    (assert (= 1 (file-position stream)))
    (assert (file-position stream :start))
    (assert (= 0 (file-position stream)))
    (assert (file-position stream :end))
    (assert (= (length octets) (file-position stream)))
    (assert (file-position stream (1- (file-position stream))))
    (assert (= (read-byte stream) 3))
    (assert (file-position stream (1- (file-position stream))))
    (assert (= (read-byte stream) 3))
    (assert (file-position stream :end))
    (let ((eof (cons nil nil)))
      (assert (eq (read-byte stream nil eof) eof)))
    (assert (file-position stream 10))
    ;; Avoid type mismatch warning when compiling of this file.
    (let ((fun (checked-compile `(lambda (stream)
                                   (file-position stream -1))
                                :allow-warnings t)))
      (assert-error (funcall fun stream) type-error))
    (assert-error (read-byte stream) end-of-file)))

(with-test (:name (:make-octets-input-stream read-sequence list))
  (let* ((octets (map '(vector (unsigned-byte 8)) 'digit-char-p "1234567890"))
         (stream (sb-ext:make-octets-input-stream octets 1 9)))
    (let ((list (list nil nil)))
      (assert (= (read-sequence list stream) 2))
      (assert (equal list '(2 3))))
    (let ((list (list nil nil nil)))
      (assert (= (read-sequence list stream :start 1 :end 2) 2))
      (assert (equal list '(nil 4 nil))))
    (let ((list (make-list 10)))
      (assert (= (read-sequence list stream :start 1) 6))
      (assert (equal list '(nil 5 6 7 8 9 nil nil nil nil))))))

(with-test (:name (:make-octets-input-stream read-sequence vector))
  (let* ((octets (map '(vector (unsigned-byte 8)) 'digit-char-p "1234567890"))
         (stream (sb-ext:make-octets-input-stream octets 1 9)))
    (let ((vector (vector nil nil)))
      (assert (= (read-sequence vector stream) 2))
      (assert (equalp vector #(2 3))))
    (let ((vector (vector nil nil nil)))
      (assert (= (read-sequence vector stream :start 1 :end 2) 2))
      (assert (equalp vector #(nil 4 nil))))
    (let ((vector (make-array 10 :initial-element nil)))
      (assert (= (read-sequence vector stream :start 1) 6))
      (assert (equalp vector #(nil 5 6 7 8 9 nil nil nil nil))))))

(with-test (:name (:make-octets-input-stream read-sequence :octet-vector))
  (let* ((octets (map '(vector (unsigned-byte 8)) 'digit-char-p "1234567890"))
         (stream (sb-ext:make-octets-input-stream octets 1 9)))
    (let ((vector (make-array 2 :element-type '(unsigned-byte 8))))
      (assert (= (read-sequence vector stream) 2))
      (assert (equalp vector #(2 3))))
    (let ((vector (make-array 3 :element-type '(unsigned-byte 8))))
      (assert (= (read-sequence vector stream :start 1 :end 2) 2))
      (assert (equalp vector #(0 4 0))))
    (let ((vector (make-array 10 :element-type '(unsigned-byte 8))))
      (assert (= (read-sequence vector stream :start 1) 6))
      (assert (equalp vector #(0 5 6 7 8 9 0 0 0 0))))))

(with-test (:name (:with-input-from-octets read-byte :octet-vector))
  (let ((octets (map '(vector (unsigned-byte 8)) 'digit-char-p "1234567890")))
    (with-input-from-octets (s octets :start 1 :end 9)
      (assert (= (read-byte s) 2))
      (assert (= (read-byte s) 3)))))

(with-test (:name (:with-input-from-octets read-sequence list :octet-vector))
  (let ((octets (map '(vector (unsigned-byte 8)) 'digit-char-p "1234567890"))
        (seq (make-list 3)))
    (with-input-from-octets (s octets :start 1 :end 9)
      (assert (= (read-sequence seq s) 3))
      (assert (equal seq '(2 3 4)))
      (assert (= (read-sequence seq s) 3))
      (assert (equal seq '(5 6 7)))
      (assert (= (read-sequence seq s) 2))
      (assert (equal seq '(8 9 7))))))

(with-test (:name (:with-input-from-octets read-sequence vector :octet-vector))
  (let ((octets (map '(vector (unsigned-byte 8)) 'digit-char-p "1234567890"))
        (seq (make-array 3 :element-type '(unsigned-byte 8))))
    (with-input-from-octets (s octets :start 1 :end 9)
      (assert (= (read-sequence seq s) 3))
      (assert (equalp seq #(2 3 4)))
      (assert (= (read-sequence seq s) 3))
      (assert (equalp seq #(5 6 7)))
      (assert (= (read-sequence seq s) 2))
      (assert (equalp seq #(8 9 7))))))

;;;; MAKE-OCTETS-OUTPUT-STREAM and WITH-OUTPUT-TO-OCTETS

(with-test (:name (:make-octets-output-stream file-position))
  (let* ((stream (make-octets-output-stream)))
    (assert (= (write-byte 1 stream) 1))
    (assert (= 1 (file-position stream)))
    (assert (file-position stream :start))
    (assert (= 0 (file-position stream)))
    (assert (file-position stream :end))
    (assert (= 1 (file-position stream)))
    (assert (file-position stream (1- (file-position stream))))
    (assert (= (write-byte 3 stream) 3))
    (assert (file-position stream (1- (file-position stream))))
    (assert (= (write-byte 4 stream) 4))
    (assert (file-position stream :end))
    (let ((octets (get-output-stream-octets stream)))
      (assert (typep octets '(simple-array (unsigned-byte 8) (*))))
      (assert (equalp octets #(4))))))

(with-test (:name (:make-octets-output-stream write-sequence list))
  (let* ((stream (make-octets-output-stream)))
    (assert (equal (write-sequence '(1 2) stream) '(1 2)))
    (assert (= (write-byte 3 stream) 3))
    (assert (equal (write-sequence '(4 5 6) stream) '(4 5 6)))
    (let ((octets (get-output-stream-octets stream)))
      (assert (typep octets '(simple-array (unsigned-byte 8) (*))))
      (assert (equalp octets #(1 2 3 4 5 6))))))

(with-test (:name (:make-octets-output-stream write-sequence list :large))
  (let ((stream (make-octets-output-stream)))
    (write-sequence (loop for i from 1 to 200 collect i) stream)
    (let ((octets (get-output-stream-octets stream)))
      (assert (typep octets '(simple-array (unsigned-byte 8) (*))))
      (assert (equalp octets (coerce (loop for i from 1 to 200 collect i) 'vector))))))

(with-test (:name (:make-octets-output-stream write-sequence vector))
  (let* ((octets (coerce (loop for i from 1 to 200 collect i) 'simple-vector))
         (stream (sb-ext:make-octets-output-stream)))
    (write-sequence octets stream)
    (let ((result (get-output-stream-octets stream)))
      (assert (typep result '(simple-array (unsigned-byte 8) (*))))
      (assert (equalp result (coerce (loop for i from 1 to 200 collect i) 'vector))))))

(with-test (:name (:make-octets-output-stream write-sequence :octet-vector))
  (let* ((octets (coerce (loop for i from 1 to 200 collect i) '(vector (unsigned-byte 8))))
         (stream (sb-ext:make-octets-output-stream)))
    (write-sequence octets stream)
    (let ((result (get-output-stream-octets stream)))
      (assert (typep result '(simple-array (unsigned-byte 8) (*))))
