...
(macrolet
    ((def (name v struct init)
       `(defmacro ,name ((var ,v &key index (start 0 start-suppliedp) end)
                         &body forms-decls &environment env)
          (let* ((dummy '#:stream) ; in case VAR is declared special
                 (offset '#:offset)
                 ;; CLHS says in WITH-INPUT-FROM-STRING:
                 ;;
                 ;;  "The input string stream is automatically closed
                 ;;   on exit from with-input-from-string, no matter
                 ;;   whether the exit is normal or abnormal. The
                 ;;   input string stream to which the variable var is
                 ;;   bound has dynamic extent; its extent ends when
                 ;;   the form is exited."
                 ;;
                 ;; In light of the second point, we need not close
                 ;; the stream if the object is stack-allocated,
                 ;; because any attempt to access it after the forms
                 ;; exit will surely crash, and there are otherwise no
                 ;; observable effects from closing the stream.  The
                 ;; choice to avoid DX-LET in some policies is
                 ;; strictly unecessary, because it is *always*
                 ;; undefined behavior to use the DX object after its
                 ;; extent ends, however it might help expose user
                 ;; code bugs by keeping the stream accessible but
                 ;; closed.
            ...
            (flet ((uwp (forms)
              ...
              `(,bind ((,dummy (%make-structure-instance
                                ,(find-defstruct-description ',struct)
                                nil)))
                     ...
                     (if index
                         `(multiple-value-bind (,var ,offset) ,ctor
                            ...
                            (multiple-value-prog1 ,(uwp forms)
                              (setf ,index (- (vector-input-stream-index ,var) ,offset))))
                         `(let ((,var ,ctor))
