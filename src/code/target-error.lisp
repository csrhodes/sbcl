  ...
  (defun map-restarts (function &optional condition (call-test-p t))
    ...
    (let ((stack *restart-test-stack*))
      (dolist (restart-cluster *restart-clusters*)
        ...
        (dovector (restarts (truly-the (simple-array (or list restart) (*)) restart-cluster))
          (if (listp restarts)
    ...
    (let ((stack *restart-test-stack*))
      (dolist (restart-cluster *restart-clusters*)
        ...
        (let ((index (mod (sxhash name) (length restart-cluster))))
          (let ((restarts (truly-the (or list restart) (aref restart-cluster index))))
            (if (listp restarts)
                (dolist (restart restarts)
...
(defun with-simple-condition-restarts (function cerror-arg datum &rest arguments)
  (let ((sb!debug:*stack-top-hint* (or sb!debug:*stack-top-hint*
        ...
        (condition (apply #'coerce-to-condition datum
                          ...
                          function
                          arguments)))
    (with-condition-restarts condition (loop for x across (car *restart-clusters*) append (ensure-list x))
