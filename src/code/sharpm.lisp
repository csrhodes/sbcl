      ...
      (let ((default-constructor
        ...
        (let ((constructor-args
          ...
          (handler-bind ((type-error
                          (lambda (c)
                            (when (and (typep (sb-kernel::type-error-context c) '(cons (eql sb-kernel::struct-context)))
                                       sb-kernel::*no-check-set-value-restart-fun*
                                       (contains-marker (type-error-datum c)))
                              (funcall sb-kernel::*no-check-set-value-restart-fun* (type-error-datum c))))))
