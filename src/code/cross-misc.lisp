...
;;; macro-writing-macro (or equivalent) generates symbols at first use
;;; that the newly-defined macro will use when *it* expands, the
;;; coalescing in genesis would lead to that inner macro no longer
;;; having the correct expansion.  In the event of truly weird
;;; behaviour of some macro leading the intrepid maintainer to this
;;; comment: try turning off uninterned symbol coalescing in genesis
;;; by redefining GET-UNINTERNED-SYMBOL.  (This is not an issue for
;;; user code, which is compiled with a normal implementation of
;;; GENSYM).
